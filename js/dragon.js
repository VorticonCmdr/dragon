...
function setProgressbar() {
  ...
  let percentProgress = Math.round(
    (crawl.data.queue.length / crawl.data.queueMaxLength) * 100,
  );
...
function processPage(html, href) {
  ...
  let metadata = {
    title: doc.title || "",
    description:
      doc.querySelectorAll('meta[name="description"]')?.[0]?.content || "",
    href: href,
    og: {
      image:
        doc.querySelectorAll('meta[property="og:image"]')?.[0]?.attributes
          ?.content?.textContent || "",
      title:
        doc.querySelectorAll('meta[property="og:title"]')?.[0]?.content || "",
      site_name:
        doc.querySelectorAll('meta[property="og:site_name"]')?.[0]?.content ||
        "",
      description:
        doc.querySelectorAll('meta[property="og:description"]')?.[0]?.content ||
        "",
    },
    robots: doc.querySelectorAll('meta[name="robots"]')?.[0]?.content || "",
    canonical: doc.querySelectorAll('link[rel="canonical"]')?.[0]?.attributes
      ?.href?.textContent,
...
async function fetchURL(link) {
  ...
  crawl.data.alreadyFetched[link] = crawl.data.alreadyFetched[link]
    ? crawl.data.alreadyFetched[link] + 1
    : 1;
...
function addPerformanceData() {
  Object.keys(crawl.data.results).forEach((link, i) => {
    try {
      ...
      crawl.data.results[link]["performance"] =
        performanceData[performanceData.length - 1].toJSON();
...
function parseActivitiesData(textData) {
  ...
  result?.data?.forEach((item, i) => {
    let url = item.URL || item.page || item.Page;
    if (!url) {
      return;
    }
    urlsTxt += `${url}\n`;
