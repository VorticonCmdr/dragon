...
function readEmbeddingsJson(file) {
  ...
  reader.addEventListener("load", (event) => {
    ...
    if (embeddingsData.length) {
      /*
      embeddingsData = embeddingsData.filter((item) => {
        return (parseInt(item['Position']) < 4);
      });
      */
...
function showCluster(d) {
  ...
  let cid = `#c${d.coordinates.x}${d.coordinates.y}`
    .replaceAll(".", "")
    .replaceAll("-", "s");
  centerNode(cid);
...
function generateMap() {
  ...
  board["labels"] = board.svg
    ...
    .join("text")
    .attr("id", (d) =>
      `t${d.coordinates.x}${d.coordinates.y}`
        .replaceAll(".", "")
        .replaceAll("-", "s"),
    )
    .attr("dy", "0.35em")
  ...
  */
  ...
  board["circles"] = board.svg
    ...
    .join("circle")
    .attr("id", (d) =>
      `c${d.coordinates.x}${d.coordinates.y}`
        .replaceAll(".", "")
        .replaceAll("-", "s"),
    )
  ...
  board.setCentroids.forEach((item, i) => {
    let cid = `#c${item.coordinates.x}${item.coordinates.y}`
      .replaceAll(".", "")
      .replaceAll("-", "s");
    let tid = `#t${item.coordinates.x}${item.coordinates.y}`
      .replaceAll(".", "")
      .replaceAll("-", "s");
    ...
    $cluster.append(`<option value="${item.cluster}">${item.keyword}</option>`);
  });
}
...
function loadVectorsFromStorage() {
  chrome.storage.local.get("vectors", function (item) {
    let vectors = JSON.parse(item.vectors);
    embeddingsData = vectors.map((data) => {
      return {
        id: data.canonical,
        keyword: data.h1,
        description: data.og_description,
        title: data.h1,
        embedding: new Float64Array(Object.values(data.embedding)),
      };
    });
  });
}
